...
仓库根目录的 `render.yaml` 已配置为 Render Starter 常驻实例，使用单个
Docker 容器提供前端和 API。创建 Blueprint 时必须在 Render 控制台填写：
...
Render 会挂载 1 GB 持久磁盘保存每日额度（IP 仅保存为不可逆摘要），因此
服务重启或重新部署不会清空当天计数。评估任务和结果仍只保存在单实例内存中，
服务重启后会丢失；这符合
受限作品集演示场景。若以后开放注册或横向扩容，应把任务和额度迁移到
Redis/PostgreSQL。
